  ...
  useEffect(() => {
    if (ethereum) {
      async function contract_interact() {
        ...
        p_contract
          .checkContribution(wallet_account)
          .then((result) => {
  ...
  async function buyTokens() {
    if (buyAmnt > 0) {
      ...
      const tx = await usdc_contract.approve(
        contract,
        ethers.utils.parseEther(buyAmnt.toString())
      );
      await tx.wait();

      const transaction = await p_contract.buyTokens(
        wallet_account,
        ethers.utils.parseEther(buyAmnt.toString())
      );
      ...
      toast.error("Buy amount must be > 0");
    }
  }
          ...
          <div className="col-lg-4">
            <div className="single-report">
              <div className="single-report-item">
                ...
                <ProgressBar
                  now={(weiRaised / (totalCap / rate)) * 100}
